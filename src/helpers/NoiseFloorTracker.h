...
#if NOISE_TRACKER_FIRST_SUB_SAMPLES < 2
  #error "NOISE_TRACKER_FIRST_SUB_SAMPLES must be >= 2: a shorter first sub-window \
would leave the seed sentinel in the ring and floorDbm() would convert it."
#endif
  ...
  // window statistic take no part in it, so signal energy cannot widen the
  // estimated noise spread.
  //
  // Measured from windowValue(), deliberately, and not from the floor estimate
  // or from any multiple of sigma. A gate that grows with sigma closes a
  // positive feedback loop under sustained near-floor traffic: each admitted
  // packet raises the spread, the wider gate admits a stronger one, and sigma
  // ratchets up until MAX_SIGMA stops it -- silently widening the CSMA margin
  // derived from sigma (NOISE_THRESHOLD_SIGMA_K * sigma) in exactly the busy
  // mesh where the operator set interference_threshold. A fixed gate removes
  // that loop. It is not a tighter numeric bound -- GATE/biasK is the same 3.0
  // that MAX_SIGMA already imposes -- and traffic *inside* the gate still
  // inflates sigma with no feedback needed, worst case about 2.35 dB for
  // packets ~6 dB above the mean. Read it as a guarantee that the set of
  // samples reaching sigma cannot grow, not as a bound on sigma itself.
  //
  // The value is biasK * MAX_SIGMA (2.835 * 3.0): never look further above the
  // window statistic than the largest mean offset the estimator may report.
  // Past a true spread of ~2 dB the noise tail overflows the gate and sigma
  // reads low; that is the deliberate trade and it is the safe direction, since
  // a low sigma fires the busy check early (absorbed by CSMA backoff) rather
...
// as it must be, since the bias is proportional to it. Indexing by occupancy
// rather than using one constant matters during warm-up: a partly filled ring
// has been minimised over fewer samples, so the full-window coefficient would
// over-correct there.
//
// Determined by simulating this exact algorithm rather than from the asymptotic
// extreme-value formula, which is not accurate at these window sizes. Getting it
// wrong misreports sigma() rather than the floor -- the coefficient cancels out
// of the floor unless a sigma clamp binds -- and sigma sets the CSMA margin, so
// it still matters.
