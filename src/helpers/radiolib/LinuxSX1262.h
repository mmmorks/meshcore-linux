...
// For the LinuxBoard definition behind `board` below: std_init() and
// rxSettings() both read board.config, so the complete type is needed here.
// Reached transitively via target.h today, but named so this header does not
