...
public:
  ...
  bool drainEvents() override {
    drain_calls++;
    // A source that cannot drain leaves its descriptor readable, so read
    // nothing on that path -- that is what makes the failure spin-shaped.
  ...
  // Simulate a persistent read error on the event descriptor.
...
TEST(WaitForIrqAsserted, DoesNotSpinWhenTheSourceCannotDrain) {
  // The sibling of the test above for the case where draining does not work:
  // the descriptor is readable, stays readable, and drainEvents() says so.
  // LinuxEventLoop::wait() has to back off for that exactly as it does for a
  // stale descriptor, or the wait degenerates into the 100%-core loop this
  // whole unit exists to remove -- silently, since a failed drain is not
  ...
  source.failDrains();
  source.signal();  // readable, and nothing will clear it
  ...
  // The discriminator. One level sample per iteration, so this counts the
  // iterations: with the 1 ms backoff it is on the order of the timeout in
  // milliseconds, and without it the loop would run as fast as poll() can
  // return -- tens of thousands of times in these 40 ms.
  EXPECT_GE(source.drain_calls, 1);
  EXPECT_LT(level.calls, 1000);
  EXPECT_GT(source.pendingBytes(), 0);  // still undrained, as in the real failure
}
