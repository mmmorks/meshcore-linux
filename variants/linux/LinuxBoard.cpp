...
// Stands in for RadioIrqLevel when no IRQ pin is configured: there is no line
// to sample, so the level never asserts and only the deadline can end the wait.
class NoIrqLevel : public LinuxIrqLevel {
public:
...
bool LinuxBoard::waitForRadioIrq(uint32_t timeout_ms) {
  // Callers read the operation's result over SPI, so a line that never reports
  // costs them the wait, not the answer -- but only if the wait actually
  // happens. Returning at once here instead would cost them the answer: the
  // caller reads a status register the modem has not had time to latch, and
  // for CAD that means SX126x::getChannelScanResult() sees neither CAD_DONE
  // nor CAD_DETECTED and returns RADIOLIB_ERR_UNKNOWN, which
  // isChannelActive() reads as busy -- every channel, every transmit.
  //
  // So the unconfigured pin waits out the deadline exactly like a dead line
  // does, just with nothing to sample and nothing to block on: NoIrqLevel
  // never asserts and irq_event_source is NULL (it is only ever set when a pin
  // is configured), leaving waitForIrqAsserted() to run its bounded 1 ms-slice
  // fallback to the deadline.
  if (config.lora_irq_pin == RADIOLIB_NC) {
    NoIrqLevel none;
    return waitForIrqAsserted(none, irqEventSource(), EventLoop, timeout_ms);
  }
...
static const bool PIN_REQUIRED = false;   // names for parse_pin()'s `optional`
static const bool PIN_OPTIONAL = true;
...
// in a way that reads as a wiring fault rather than a typo. On a bad value the
...
//
// `optional` marks a pin that may be explicitly unset, which is only ever the
// RF-switch pair: there is no such thing as an unset NSS line. It accepts the
// clear spelling `none` and also `-1`, which is what the previous atoi() turned
// into RADIOLIB_NC by accident and therefore what deployed configs contain --
// rejecting it would take a working node off the air on upgrade.
static bool parse_pin(const char *key, const char *value, bool optional, long *out, int *bad_values) {
  if (optional && (strcmp(value, "-1") == 0 || strcasecmp(value, "none") == 0)) {
    *out = (long) RADIOLIB_NC;
    return true;
  }
  char *end = NULL;
  long v = strtol(value, &end, 10);
  if (end == value || *end != '\0' || v < 0 || v > 255) {
    printf("ERROR: meshcored.ini: %s = '%s' is not a valid GPIO pin (expected 0..255%s)\n",
           key, value, optional ? ", or -1/none for unused" : "");
    (*bad_values)++;
    return false;
  }
  *out = v;
...
//
// An empty value is a bad value, exactly like the numeric helpers treat one
// (`strtof`/`strtol` with `end == value`). Without this, a truncated line such
// as a bare `admin_password` -- key, no `=`, no value -- would set the field to
// "" with nothing said, and MyMesh::begin() would then persist that empty
// password to prefs.json on the first boot. The compile-time default is left in
// place, and begin() refuses to start on the count.
static void assign_string(const char *key, char *value, const char **field, bool *owned,
                          size_t maxlen, int *bad_values) {
  if (*value == '\0') {
    printf("ERROR: meshcored.ini: %s = '%s' is empty (expected a value)\n", key, value);
    (*bad_values)++;
    return;
  }
...
LinuxConfig::LoadResult LinuxConfig::load(const char *filename) {
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    // discard the whole line rather than guess where it was meant to end.
    //
    // Swallow the remainder here, but hold the verdict: whether this is fatal
    // depends on what the line was, and that is only known after the
    // comment/blank/section test below. A comment or a section header long
    // enough to hit the limit is still inert, and taking a repeater off the air
    // over a long *comment* would be indefensible.
    //
    // Note the limit is reached, not necessarily exceeded: a line of exactly
    // sizeof(line)-1 characters followed by its newline is indistinguishable
    // from a longer one without reading ahead, and is discarded too. The
    // message says "reached" for that reason.
    bool overlong = false;
    ...
    // Not a comment, a blank or a section header, so this was meant to be a
    // setting -- and the part of it that was read cannot be trusted to be the
    // whole of it.
    if (overlong) {
      printf("ERROR: meshcored.ini: line reached the %zu-byte limit; discarding it\n",
             sizeof(line) - 1);
      result.bad_values++;
      continue;
    }
    ...
    if (*p == '\0') {
      ...
      // consumes it), so counted -- and labelled -- the same way rather than
      // treated as fatal.
      printf("WARNING: meshcored.ini: line '%s' has no '=' (ignored)\n", trim(key));
      result.unknown_keys++;
      continue;
    }
    *p++ = '\0';
    ...
    *p = '\0';
    ...
    if (strcmp(key, "spidev") == 0) {
      ...
      if (parse_float(key, value, &fval, &result.bad_values)) lora_bw = fval;
    } else if (strcmp(key, "lora_sf") == 0) {
      if (parse_int(key, value, 0, 255, &ival, &result.bad_values)) lora_sf = (uint8_t) ival;
    } else if (strcmp(key, "lora_cr") == 0) {
      if (parse_int(key, value, 0, 255, &ival, &result.bad_values)) lora_cr = (uint8_t) ival;
    } else if (strcmp(key, "lora_tcxo") == 0) {
      if (parse_float(key, value, &fval, &result.bad_values)) lora_tcxo = fval;
    } else if (strcmp(key, "lora_tx_power") == 0) {
      if (parse_int(key, value, -128, 127, &ival, &result.bad_values)) lora_tx_power = (int8_t) ival;
    } else if (strcmp(key, "current_limit") == 0) {
      if (parse_float(key, value, &fval, &result.bad_values)) current_limit = fval;
    } else if (strcmp(key, "dio2_as_rf_switch") == 0) {
      ...
      if (parse_pin(key, value, PIN_REQUIRED, &pin, &result.bad_values)) lora_reset_pin = (uint32_t) pin;
    } else if (strcmp(key, "lora_nss_pin") == 0) {
      if (parse_pin(key, value, PIN_REQUIRED, &pin, &result.bad_values)) lora_nss_pin = (uint32_t) pin;
    } else if (strcmp(key, "lora_busy_pin") == 0) {
      if (parse_pin(key, value, PIN_REQUIRED, &pin, &result.bad_values)) lora_busy_pin = (uint32_t) pin;
    } else if (strcmp(key, "lora_rxen_pin") == 0) {
      if (parse_pin(key, value, PIN_OPTIONAL, &pin, &result.bad_values)) lora_rxen_pin = (uint32_t) pin;
    } else if (strcmp(key, "lora_txen_pin") == 0) {
      if (parse_pin(key, value, PIN_OPTIONAL, &pin, &result.bad_values)) lora_txen_pin = (uint32_t) pin;
    } else if (strcmp(key, "advert_name") == 0) {
      assign_string(key, value, &advert_name, &advert_name_owned, 100, &result.bad_values);
    } else if (strcmp(key, "admin_password") == 0) {
      assign_string(key, value, &admin_password, &admin_password_owned, 100, &result.bad_values);
      ...
      // warns about these and refuses to start over those. WARNING, not ERROR,
      // so this line and the summary two lines later agree about what happened.
      printf("WARNING: meshcored.ini: unknown key '%s' (ignored)\n", key);
      result.unknown_keys++;
    }
  }
  fclose(f);
