...
public:
  ...
  // Buffer size for a gps_device string, terminator included. Also the limit
  // LinuxConfig::load() validates against, so the two cannot drift: the stable
  // names the ini templates recommend are long -- real /dev/serial/by-id/ paths
  // measure 74 and 81 characters for a u-blox and a Prolific adapter.
  static constexpr size_t DEVICE_MAX = 256;

  LinuxGpsStream() = default;

  // The open descriptor is this object's whole state, so it needs the same
  // treatment PtyConsole gives its own: closed on destruction, and never
  // duplicated into a second owner that would close it twice.
  ~LinuxGpsStream() { end(); }
  LinuxGpsStream(const LinuxGpsStream&) = delete;
  LinuxGpsStream& operator=(const LinuxGpsStream&) = delete;
  ...
  //                    would be a lie. Also false in the window after the
  //                    device disappeared and before serviceSerial() has it
  //                    open again, which is the same answer for the same
  //                    reason.
...
protected:
  ...
  // inspect it (that it is close-on-exec), while any poll-based idle loop must
  // still have no way to reach it. EnvironmentSensorManager drains this stream
  // only while GPS is active, but the module streams regardless, so a
  // descriptor registered with a poll() loop that nothing drains would sit
  // permanently readable and turn that loop into a busy spin.
...
private:
  ...
  // SERIAL_DEVICE only: what begin() was given, kept so the device can be
  // reopened after it disappears (a USB receiver unplugged and replugged).
  char _dev_path[DEVICE_MAX] = "";
  int  _dev_baud = 0;
