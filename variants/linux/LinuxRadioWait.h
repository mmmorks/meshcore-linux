...
//
// Never spins. Each iteration blocks in poll() on src's edge descriptor for the
// whole remaining time; where src has no usable descriptor it waits in 1 ms
// slices instead, the same fallback (and the same value) as
...
// registered: nothing here would drain a console or GPS descriptor, and a
// registered-but-undrained descriptor stays POLLIN forever, which is precisely
// the busy loop LinuxEventLoop exists to remove.
