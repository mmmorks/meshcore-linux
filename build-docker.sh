...
# minimal repro. Scoping to this run's own paths avoids ever touching another
# env's tree; `|| echo ... WARNING` on top of that means even an unexpected
# chown failure inside the scoped paths is reported but never turns a
# successful build into a script failure.
#
# The three .pio parent directories get a separate NON-recursive chown. On a
# fresh checkout on a Linux host the container is what creates .pio, the build
# dir and .pio/libdeps, so they are left owned by root -- and then the invoking
# user cannot create a sibling directory in them, so a subsequent plain
# `pio test -e native` fails with EACCES trying to make .pio/build/native.
# Non-recursive keeps the narrow scoping above intact: it fixes the three
# directory inodes and touches nothing inside them.
docker run --rm \
  ...
  "${IMAGE}" bash -c '
    ...
    # Neither platformio.ini reads FIRMWARE_VERSION, so a bare pio run leaves
    # the #ifndef FIRMWARE_VERSION default in examples/simple_repeater/MyMesh.h
    # standing and meshcored -V then reports that instead of the version this
    # script was asked for. PlatformIO appends PLATFORMIO_BUILD_FLAGS to the
    # build_flags of the env, so define it here the same way build.sh does for
    # the MCU targets: the inner double quotes are what make the value reach
    # C++ as a string literal instead of a bare identifier.
    #
    # The backslash-escaped single quotes below are the usual way to put a
    # literal quote inside this single-quoted bash -c argument; a bare one
    # would end the script early and let the HOST expand FIRMWARE_VERSION.
    ...
    chown "${HOST_UID}:${HOST_GID}" .pio "${PLATFORMIO_BUILD_DIR}" .pio/libdeps 2>/dev/null || true
  '
