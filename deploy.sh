...
#
# Copies the binary, meshcorectl and gnssctl over, installs them, restarts
...
#     The config is node-specific and holds the admin password. The unit may
#     have been customised -- on a real node it had a drop-in adding
#     RuntimeDirectory, because the installed copy predated that line.
#
#   * Install or configure gpsd and chrony for a gpsd:// GPS source. Whether a
#     node uses one is a property of its meshcored.ini, which this script does
...
scp "${SSH_OPTS[@]}" -q \
  ...
  "$REPO_DIR/variants/linux/gnssctl" \
  "$REPO_DIR/variants/linux/meshcored.service" \
  "$HOST:$STAGE/"

# --- 4. set up and install ----------------------------------------------------
#
# Idempotent setup, then two binaries and a restart.
#
...
# gnssctl is both a diagnostic and the receiver's boot-time initialiser -- the
# gpsd drop-in's ExecStartPre calls it, so unlike the gnss-probe it replaces it
# is part of the running system.
sudo install -m 755 "$STAGE/gnssctl"     /usr/bin/gnssctl
